...
<div class="relative min-h-screen bg-slate-950">
    <div class="pointer-events-none absolute inset-0 overflow-hidden">
        <div class="absolute -top-40 -right-20 h-96 w-96 rounded-full bg-orange-500/20 blur-3xl"></div>
        <div class="absolute top-72 -left-32 h-[420px] w-[420px] rounded-full bg-amber-500/10 blur-3xl"></div>
        <div class="absolute bottom-[-200px] right-16 h-[480px] w-[480px] rounded-full bg-yellow-500/10 blur-3xl"></div>
    </div>

    <div class="relative mx-auto max-w-7xl px-4 py-10 text-slate-100 sm:px-6 lg:px-8 lg:py-14">
        <div class="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
            <div class="space-y-2">
                <div class="inline-flex items-center gap-2 rounded-full border border-orange-400/40 bg-orange-400/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.3em] text-orange-200">
                    <span class="h-2 w-2 animate-pulse rounded-full bg-orange-300"></span>
                    Priority Rank 3
                </div>
                <h1 class="text-3xl font-extrabold tracking-tight text-white sm:text-4xl">Certificate of Tribal Membership</h1>
                <p class="max-w-2xl text-sm text-slate-300">
                    Students who have successfully uploaded and been approved for their Certificate of Tribal Membership. This is the third priority document in the scholarship application process.
                </p>
            </div>
        ...
        <div class="mt-10 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <div class="group relative overflow-hidden rounded-2xl border border-white/10 bg-white/5 p-5 shadow-lg backdrop-blur transition hover:-translate-y-1 hover:border-orange-300/40">
                <div class="absolute -right-8 -top-8 h-20 w-20 rounded-full bg-orange-400/20 blur-3xl transition group-hover:scale-110"></div>
                <p class="text-xs uppercase tracking-[0.25em] text-orange-200">Total Approved</p>
                <div class="mt-2 text-3xl font-bold text-white">{{ $totalApproved ?? 0 }}</div>
                <p class="text-xs text-orange-100/70">Students with approved certificates</p>
            </div>
            ...
            <div class="group relative overflow-hidden rounded-2xl border border-white/10 bg-white/5 p-5 shadow-lg backdrop-blur transition hover:-translate-y-1 hover:border-amber-300/40">
                ...
                <p class="text-xs uppercase tracking-[0.25em] text-amber-200">Priority Rank</p>
                <div class="mt-2 text-3xl font-bold text-white">#3</div>
                <p class="text-xs text-amber-100/70">Third priority document</p>
            </div>
            ...
            <div class="mt-10 overflow-hidden rounded-3xl border border-white/10 bg-white/5 shadow-xl backdrop-blur">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-white/10 text-sm text-slate-100">
                        <thead class="bg-orange-500/20 text-xs uppercase tracking-[0.25em] text-orange-100">
                            <tr>
                                <th class="px-5 py-4 text-left">#</th>
                                <th class="px-5 py-4 text-left">Applicant</th>
                                <th class="px-5 py-4 text-left">Indigenous Group</th>
                                <th class="px-5 py-4 text-left">Certificate Approved</th>
                            ...
                            @foreach($prioritizedUsers as $index => $user)
                                ...
                                <tr class="transition hover:bg-orange-400/10">
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                @if($user->basicInfo && $user->basicInfo->type_assist)
                                                    <p class="text-[10px] text-orange-300/70 mt-1">{{ $user->basicInfo->type_assist }}</p>
                                                @endif
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-5 py-4 text-right text-sm">
                                        <a href="{{ route('staff.applications.view', $user->id) }}" class="inline-flex items-center gap-2 rounded-full border border-orange-300/40 bg-orange-300/15 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-orange-100 transition hover:bg-orange-300/25">View →</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
        ...
        @else
            <div class="mt-10 rounded-2xl border border-white/10 bg-white/5 p-12 text-center text-sm text-slate-200/70">
                <svg class="mx-auto mb-4 h-14 w-14 text-orange-200/40" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                No students with approved Certificate of Tribal Membership found. Students will appear here once they upload and their certificate is approved by staff.
            </div>
        @endif
    </div>
</div>
